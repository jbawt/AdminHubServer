...
module.exports = (db) => {
  ...
  router.post('/new-folder', (req, res) => {
    ...
    drive.files.create({
      resource: metaData,
      fields: 'id, name, size, modifiedTime, mimeType, createdTime, owners, fileExtension, thumbnailLink, webViewLink, viewedByMeTime, webContentLink',
    }, (error, file) => {
      if (error) {
        console.log(error);
      } else {
  ...
  //   const { folder, newFile } = req.body;
  //   const { metaData, media } = newFile;
