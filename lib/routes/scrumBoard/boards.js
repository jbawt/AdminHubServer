    ...
    if (verified) {
      ...
      const boardData = await db.query('INSERT INTO boards (name, uri) VALUES ($1, $2) RETURNING *;', [name, uri])
        ...
        if (claim.role === 'admin') {
          ...
          db.query('INSERT INTO board_settings (board_id, color, subscribed, card_cover_images) VALUES ($1, $2, $3, $4) RETURNING *;', [resObj.id, settings.color, settings.subscribed, settings.cardCoverImages])
            .then(data => {
              resObj.settings = data.rows[0];
            })
            .then(() => {
              db.query('INSERT INTO board_members (user_id, board_id) VALUES ($1, $2);', [claim.id, resObj.id])
                .then(() => {
