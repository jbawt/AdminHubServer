...
module.exports = (db) => {
  ...
  router.post('/card/new', async (req, res) => {
    ...
    const boardInfo = await db.query('SELECT * FROM boards WHERE id = $1;', [boardId])
        ...
        // CARDS
        const boardCards = await db.query('SELECT id, name, description, id_attachment_cover, subscribed, due, draggable_id FROM cards;')
  ...
  router.post('/card/remove', (req, res) => {
    ...
    db.query('SELECT draggable_id FROM cards WHERE id = $1;', [cardId])
      .then(data => {

        const draggableId = data.rows[0].draggable_id;

        db.query('DELETE FROM cards WHERE id = $1;', [cardId])
          .then(() => {
            res.json({ cardId, draggableId });
          })
          .catch(err => console.log(err));
      })
      .catch(err => console.log(err));
  ...
  router.post('/card/update', async (req, res) => {
    ...
    const { id, name, description, idMembers, idLabels, attachments, checklists, activities, due, draggableId } = card;
    ...
    if (currentAttachments.rows.length < attachments.length) {
      // add attachment
    ...
    if (newChecklists.length > currentChecklists.rows.length) {
      ...
      const updatedChecklists = await Promise.all(checklists.map(async (checklist) => {
        ...
        if (checklist.checkItems.length < currentChecklistItems.rows.length) {
          ...
          // UPDATING ITEM
          const checklistItems = newChecklistItems.length > 0 ? newChecklistItems : checklist.checkItems;
  ...
  router.post('/card/remove-attachment', (req, res) => {
    ...
    db.query('DELETE FROM attachments WHERE id = $1;', [attachmentId])
      .then(() => {
        db.query('SELECT id_attachment_cover FROM cards WHERE id = $1;', [cardId])
          .then(data => {
            if (data.rows[0].id_attachment_cover === null) {
              res.json(attachmentId);
            } else {
              db.query('UPDATE cards SET id_attachment_cover = $1 WHERE id = $2;', [null, cardId])
                .then(() => {
                  res.json(attachmentId);
                })
                .catch(err => console.log(err));
  ...
  router.post('/card/subscribe', async (req, res) => {

    const { cardId, subscribed, boardId } = req.body;
    const { authorization } = req.headers;
    const token = authorization.split(" ")[1];
    const claim = jwt.decode(token);

    await db.query('UPDATE card_members SET subscribed = $1 WHERE card_id = $2 AND board_member_id = (SELECT id FROM board_members WHERE user_id = $3 AND board_id = $4);', [subscribed, cardId, claim.id, boardId]);

    res.json({ cardId, subscribed })
