...
module.exports = (db) => {
  ...
  router.get('/get-chat', async (req, res) => {
    ...
    try {
      ...
      const userChatList = await db.query('SELECT id, contact_id, user_id, last_message_date FROM user_chats WHERE user_id = $1 OR contact_id = $1;', [userId]);
      ...
      const chatId = userChatList.rows.map(chat => {
        if (chat.contact_id === parseInt(contactId) || chat.contact_id === parseInt(userId)) {
          return chat.id;
        } else {
          return null;
        }
