  ...
  router.get('/user', async (req, res) => {
    ...
    if (verified) {
      ...
      try {
        ...
        const chatList = await db.query('SELECT id, contact_id, user_id, last_message_date FROM user_chats WHERE user_id = $1 OR contact_id = $1;', [claim.id]);
        const chatListArr = chatList.rows.map(chat => {
          return {
            chatId: chat.id,
            contactId: chat.contact_id === claim.id ? chat.user_id : chat.contact_id,
